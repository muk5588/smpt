...
<div class="container form-container">
    <h2 class="form-title">Q&A 수정</h2>
    <hr>
    <c:if test="${not empty sessionScope.dto and sessionScope.dto.userno != 0}">
        <c:if test="${not empty message}">
            <div class="alert alert-danger" role="alert">${message}</div>
        </c:if>
        <form action="${pageContext.request.contextPath}/qanda/update/${qanda.seq}" method="post" enctype="multipart/form-data" onsubmit="return submitEditorContent();">
            <div class="mb-3">
                <label for="title" class="form-label">제목:</label>
                <input type="text" id="title" name="title" class="form-control" required value="${qanda.title}">
            </div>
            <div class="mb-3">
                <label for="content" class="form-label">내용:</label>
                <textarea id="content" name="content" style="display:none;">${qanda.content}</textarea>
                <textarea id="editor" style="width:100%; height:300px;">${qanda.content}</textarea>
            </div>
            <div class="mb-3">
                ...
                <c:if test="${not empty qanda.imagePath}">
                    <img src="${pageContext.request.contextPath}${qanda.imagePath}" alt="기존 이미지" class="img-fluid mt-3">
                </c:if>
            </div>
            <div class="mb-3">
                ...
                <c:if test="${not empty qanda.attachmentPath}">
                    <a href="${pageContext.request.contextPath}${qanda.attachmentPath}" class="btn btn-info mt-2" download>기존 첨부파일 다운로드</a>
                </c:if>
            </div>
