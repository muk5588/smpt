    ...
    <div class="container">
        ...
        <table>
            ...
            <c:choose>
                ...
                <tr>
                    <td class="checkbox"><input type="checkbox" value="${qanda.boardNo }" name="deleteNum"
                                                class="delCheckBox"></td>
                    <td class="no">${qanda.boardNo }</td>
                    <td class="title">
                    <c:if test="${not empty param.categoryNo }">
						<a href="./view?categoryNo=${param.categoryNo}&boardNo=${qanda.boardNo}&curPage=${curPage}">${qanda.title}</a>
					</c:if>
                    <c:if test="${empty param.categoryNo }">
						<a href="./view?&boardNo=${qanda.boardNo}&curPage=${curPage}">${qanda.title}</a>
					</c:if>
                    </td>
                    <td class="nick">${qanda.nickName }</td>
                    <td class="hit">${qanda.boardView }</td>
                    <td class="date">
                        <fmt:formatDate value="${qanda.createDate }" pattern="yyyy-MM-dd"/>
                    </td>
                    <c:forEach items="${totalrecomm }" var="recommList">
                        <c:if test="${recommList.BOARDNO eq qanda.boardNo }">
