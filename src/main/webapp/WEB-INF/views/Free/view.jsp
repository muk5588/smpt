...
//                     }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res) {
                    $(function () {
                        $(location).attr('href', './view?categoryNo=${param.categoryNo}&boardNo=${qanda.boardNo}')
                    })
                }
            },
            error: function () {
...
<body>
	...
	<div class="wrap mx-auto">
		...
		<div class="container">
			...
			<c:choose>
				<c:when test="${usrno != 0 }">
					<a href="./userbyqandalist?userno=${dto1.userno}">
						<button>목록으로</button>
					</a>
				</c:when>
				<c:otherwise>
			...
			<c:if test="${qanda.userNo == dto.userno }">
				<a href="./update?boardNo=${qanda.boardNo }">
					<button id="btnUpdate">수정</button>
				</a>
				<a href="./delete?boardNo=${qanda.boardNo }">
			...
			<table class="table table-striped table-hover table-sm">
				...
				<tr>
					<td class="no">${qanda.boardNo }</td>
					<td class="title">${qanda.title }</td>
					<!-- HTML을 이스케이프하지 않고 그대로 렌더링하기 위해 c:out의 escapeXml 속성을 false로 설정 -->
					<%-- <td class="content">${fn:escapeXml(board.content)}</td> --%>
					<td class="content">${qanda.content }<!-- 첨부된 파일을 보여주는 부분 추가 -->
					...
					<td class="nick">${qanda.nickName }</td>
					<td class="hit">${qanda.boardView }</td>
					<td class="date"><fmt:formatDate value="${qanda.createDate }"
			...
			<!-- 게시글 수정 기능 -->
			<c:if test="${isLogin > 0 and qanda.userNo == dto1.userno }">
				<a href="./update?boardNo=${qanda.boardNo }">
					<button id="btnUpdate">수정</button>
				</a>
			</c:if>
			...
			<c:if test="${isLogin > 0}">
				...
				<button
					onclick="location.href='../report/boardReport?categoryNo=${param.categoryNo}&boardno=${qanda.boardNo}'">
					신고하기</button>
			</c:if>
			<hr>
			<div class="comment">
				<table border="1px" style="width: 80%; text-align: center;">
					...
					<c:forEach var="comment" items="${comment }">
						<tr>
							...
							<c:if test="${isLogin > 0 and qanda.userNo == comment.userNo }">
								<td>
