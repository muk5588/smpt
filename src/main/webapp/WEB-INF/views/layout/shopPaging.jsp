...
<div>
   ...
   <c:when test="${empty paging.search and empty param.userno }">
    ...
    <c:when test="${not empty paging.search or (not empty paging.minPrice or not empty paging.maxPrice) and empty param.userno}">
        ...
        <c:if test="${paging.curPage ne 1 }">
            <li class="page-item">
                <a class="page-link" href="${URL}?curPage=1&?search=${paging.search }&minPrice=${paging.minPrice}&maxPrice=${paging.maxPrice}">&larr; 처음</a>
            </li>
        </c:if>
        ...
        <c:choose>
            <c:when test="${paging.startPage ne 1 }">
                <li class="page-item">
                    <a class="page-link" href="${URL}?curPage=${paging.startPage - paging.pageCount }&search=${paging.search }&minPrice=${paging.minPrice}&maxPrice=${paging.maxPrice}">&laquo;</a>
                </li>
            </c:when>
            <c:when test="${paging.startPage eq 1 }">
                <li class="page-item">
        ...
        <c:choose>
            <c:when test="${paging.startPage ne paging.totalPage }">
                <li class="page-item">
                    <a class="page-link" href="${URL}?curPage=${paging.startPage + paging.pageCount }&search=${paging.search }&minPrice=${paging.minPrice}&maxPrice=${paging.maxPrice}">&raquo;</a>
                </li>
            </c:when>
            <c:when test="${paging.startPage eq paging.totalPage }">
                <li class="page-item">
        ...
        <c:if test="${paging.curPage ne paging.totalPage }">
            <li class="page-item">
                <a class="page-link" href="${URL}?curPage=${paging.totalPage }&search=${paging.search }&minPrice=${paging.minPrice}&maxPrice=${paging.maxPrice}">끝 &rarr;</a>
            </li>
        </c:if>
    </ul>
</c:when>
